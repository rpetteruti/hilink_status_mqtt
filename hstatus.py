...
def print_connection_status(device_ip, token):
    ...
    battery_percent = d['response']['BatteryPercent']
    ...
    if connection_status == '901':
        ...
        mqttc.username_pw_set("python_pub", "python_pub0987")
        mqttc.connect("127.0.0.1", 8883)
        mqttc.publish("sensor/modem/network_type", network_type,retain=False)
        mqttc.publish("sensor/modem/signal_level", signal_level,retain=False)
        mqttc.publish("sensor/modem/network_type_str", str(get_network_type(network_type)),retain=False)
        mqttc.publish("sensor/modem/battery_percent", str(battery_percent),retain=False)
        mqttc.loop(2)
...
status,result = sp.getstatusoutput("ping -c1 -w2 192.168.8.1" )
if status == 0:
    print("System 192.168.8.1 is UP !")
else:
    print("System 192.168.8.1 is DOWN !")
    mqttc = mqtt.Client("user_mqtt")
    mqttc.username_pw_set("user_mqtt", "password_user_mqtt")
    mqttc.connect("127.0.0.1", 8883)
    mqttc.publish("sensor/modem/network_type", '-1',retain=False)
    mqttc.publish("sensor/modem/signal_level", '-1',retain=False)
    mqttc.publish("sensor/modem/network_type_str", str(get_network_type('pippo')),retain=False)
    mqttc.loop(2)
    sys.exit(-1)
